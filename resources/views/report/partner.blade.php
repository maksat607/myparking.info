    ...
    <div class="container page-head-wrap">
        ...
        <form action="{{ route('report.report-by-partner') }}" method="GET" class="filter">
            <div class="d-flex align-items-center mb-3">
                ...
                <label class="field-style">
                    <span>Статус</span>
                    <select name="status_id" class="page-select" id="time-interval">
                        <option value="instorage">Хранение</option>
                        <option @if(request()->query('status_id') == 'arrived') selected @endif  value="arrived">Были на
                            Хранение
                        </option>
                ...
                <label class="field-style">
                    ...
                    <input type="text" class="date-range input" name="dates" id="date-range-input" value="">
                    @if(request()->get('status_id') && request()->get('status_id') != 'instorage')
                                        @push('scripts')
                ...
                <label class="chosen-post">
                    ...
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            ...
                            fill="#536E9B"/>
                        <path d="M9 9L3.5 10L7.5 14.5L7 20.5L12 18L17.5 20.5L16.5 14L21 10L15 9L12 4L9 9Z"
                              fill="transparent"/>
                    </svg>
                </label>
            </div>
            ...
            <table class="table fs-13 sortable horizontal-scrollable vertical-scrollable">
                ...
                @foreach($data['data'] as $application)
                    ...
                    <tr class="car-show-modal" data-app-id="{{ $application['id'] }}">
                        ...
                        @isset($application['parked_days_in_period'])
                            <td style="word-break: keep-all">{{ $application['parked_days_in_period'] }}</td>
                        @endisset
                        @isset($application['parked_price_in_period'])
                            <td style="word-break: keep-all">{{ $application['parked_price_in_period'] }}</td>
                        @endisset
                    </tr>
