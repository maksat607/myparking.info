    ...
    <ul class="page-nav">
        ...
        <li class="notification page-nav__item{{ (request()->routeIs('issue_requests.index')) ? ' active' : '' }}">
            <a href="{{ route('issue_requests.index',$filters) }}" class="page-nav__link">На
                выдачу</a>
            <div class="notification__count mk">{{ $totals['issue_requests'] }}</div>
        </li>
        <li class="notification page-nav__item{{ (request()->route('status_id') == 9) ? ' active' : '' }}">
